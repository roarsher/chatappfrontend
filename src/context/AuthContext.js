...
export const AuthProvider = ({ children }) => {
  ...
  // ─── On mount: restore session via Bearer token (survives refresh) ──────────
  // If cookie works  → /auth/me succeeds via cookie
  // If cookie blocked → Authorization header (from sessionStorage token) is used
  const checkAuth = useCallback(async () => {
    try {
      const { data } = await api.get('/auth/me');
      setUser(data.user);
  ...
  const register = async (username, email, password) => {
    const { data } = await api.post('/auth/register', { username, email, password });
    if (data.token) setToken(data.token);   // persist across refresh
    setUser(data.user);
    return data;
  };
